...
class vrouter(object):
    ...
    def getMAC(self, packet, nexthop, dip):
        ...
        #newA = struct.unpack("2s2s1s1s2s6s4s6s4s", a)

        #arp header is fuckkked up double check everything compared to github
        ##########################################
        #newA = []
        #newA.append('\x00\x01')
        #newA.append('\x08\x00')
        ...
        if myip == '':
            sys.exit(-1)
        aheader[6] = socket.inet_aton(myip)
        aheader[7] = self.mactobinary('00:00:00:00:00:00')
        #aheader[7] = self.mactobinary('ff:ff:ff:ff:ff:ff')
